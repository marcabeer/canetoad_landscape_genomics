...
# Read in treemix files for OptM
optM_read <- function(folder, orientagraph){
...
#' Read TreeMix output files for plotting
#' 
...
plot_treemix <- function(obj, plot.nodes = TRUE, plot.migration = TRUE, branch.colour = "grey", label = TRUE, ...) {
  ...
  if (plot.migration && nrow(subset(stuff$edges, type == "MIG"))) {
    p <- p + ggplot2::geom_curve(data = subset(stuff$edges, type == "MIG"),
      ...
      ggplot2::scale_colour_gradient2("Migration\nweight", high = "orange", mid = "blue", limits=c(0,0.5))
  }
  
  if (label)
    p <- p + ggplot2::geom_text(ggplot2::aes(x = xo, y = y, label = pop), hjust = 0, size=3)
  
  if (plot.nodes)
    p <- p + ggplot2::geom_point(data = d, ggplot2::aes(x = x, y = y), colour="gray30", size=1)
