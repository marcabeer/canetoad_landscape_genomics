...
##############################
#Add NeEstimator output
##############################

#read in NeEstimator results
...
#attach locality Ne estimates to div_pops df
div_pops$ne <- ne_maf001$ne
...
#ne vs colonization year
ne_neutral_scatter <- ggscatter(data=div_pops, x='year', y='ne',
...
ne_neutral_box<-ggplot()+
  geom_half_boxplot(data=div_pops, aes(x=region_name, y=ne, fill=region_name), outlier.shape=4, outlier.size=2, width=0.5)+
  scale_fill_manual(values=pal)+
  geom_half_point(data=div_pops, aes(x=region_name, y=ne, shape=region_name, color=region_name), alpha=0.5, size=2.5, position="dodge2", transformation=position_jitter(width=0.075, height=0))+
...
ne_neutral_box + ne_neutral_scatter + ar_box + ar_scatter + ho_box + ho_scatter + fis_box + fis_scatter +
  plot_layout(widths=c(7,10)) + plot_layout(guides="collect") + plot_annotation(tag_levels="A") & theme(plot.tag = element_text(size = 25)) & theme(legend.position="right")
